...
public class RequestParams {
    ...
    private List< FileBody> fileParamsList;
    private List<InputStreamBody> streamParamsList;
    ...
    public void addFileParameter(String key, File file,String fileName, String mimeType, String charset){
        if(fileParamsList == null){
            fileParamsList = new ArrayList<FileBody>();
        }
        FileBody fileBody = new FileBody(file,fileName, mimeType, charset);
        fileBody.setKey(key);
        fileParamsList.add(fileBody);
    }



    public void addStreamParameter(String key, InputStream stream, long length, String fileName, String mimeType){
        if(streamParamsList == null){
            streamParamsList = new ArrayList<InputStreamBody>();
        }
        InputStreamBody streamBody = new InputStreamBody(stream, length, fileName, mimeType);
        streamBody.setKey(key);
        streamParamsList.add(streamBody);
    }

    public void addStreamParameter(  InputStreamBody streamBody){
        if(streamParamsList == null){
            streamParamsList = new ArrayList<InputStreamBody>();
        }
        streamParamsList.add(streamBody);
    ...
    public HttpEntity getEntity() {
        ...
        if (fileParams != null && !fileParams.isEmpty()) {
        ...
        }else if((fileParamsList != null && !fileParamsList.isEmpty()) ||(streamParamsList != null && !streamParamsList.isEmpty())){
            MultipartEntity multipartEntity = new MultipartEntity(HttpMultipartMode.STRICT, null, Charset.forName(charset));
            ...
            if(fileParamsList != null){
                for (FileBody entry : fileParamsList) {
                    multipartEntity.addPart(entry.getKey(), entry);
                }
            }

            if(streamParamsList != null){
                for(InputStreamBody entry : streamParamsList){
                    multipartEntity.addPart(entry.getKey(), entry);
                }
            }
            result = multipartEntity;
